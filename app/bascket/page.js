...
// CartItem Component
const CartItem = ({ cartItem, product, increaseQuantity, decreaseQuantity, removeItem }) => (
  <div className="border p-4 mb-4 max-laptop-l:flex-col flex items-center w-full justify-around border-sky-400">
    <div className="max-Wide-mobile-s:mb-3 max-laptop-l:w-full max-laptop-l:justify-start flex flex-row items-center justify-between">
      ...
      />
      <div className="max-laptop-l:mr-3 text-sky-700 max-laptop-l:text-2xl font-bold mr-5 w-[100%] max-desktop-l:text-lg">
        {`${cartItem.model}-${product.onvan}`}
      </div>
    </div>
    <div className="flex-wrap flex w-[50%] max-laptop-l:w-full flex-row justify-around items-center">
      <div className="mx-3 max-mobile-xlk:my-5 justify-center items-center text-sky-900 text-2xl max-Wide-mobile-4xl:text-lg font-semibold flex flex-row">
        {Math.floor((product.money / 100) * (100 - product.offer) / 1000) * 1000}
        <span className="mx-2 flex justify-center items-center text-lg max-Wide-mobile-4xl:text-xs">تومان</span>
        <div
          className="w-6 h-6 mr-6 rounded-full border border-black"
...
const PriceSummary = ({ totalMoney, postMoney, onCheckout }) => (
  <div className="bg-white p-8 w-[700px] shadow-xl flex flex-col justify-between items-center max-tablet-xl:w-11/12">
    ...
    <input
      ...
      className="pr-6 w-11/12 h-14 rounded-full mt-5 bg-gray-200"
    />
    <button
      onClick={() => onCheckout(totalMoney + postMoney)}
...
export default function Page() {
  ...
  return (
    <div className="text-2xl p-5 font-dorna w-full items-center justify-center">
      ...
      <div className="w-full mt-10 flex justify-between items-center flex-row text-slate-400">
      ...
      <div className="w-full max-Wide-mobile-xl:text-lg flex flex-row-reverse max-desktop-l:flex-col justify-around items-center mt-24">
        <PriceSummary totalMoney={totalMoney} postMoney={postMoney} onCheckout={handleCheckout} />
