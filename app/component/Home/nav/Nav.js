...
export default function Nav({ targetRef }) {
  ...
  useEffect(() => {
    ...
    Promise.all(
      cartItems.map((item) => axios.get(`${apiKey.getoneitem}/${item.id}`))
    )
      .then((responses) => {
        ...
        if (item.k === k) {
          ...
          product.devaiceOK?.forEach((variant) => {
            if (variant.name === item.model) {
              const stock = variant.mojodi?.[item.indexcolor];
  ...
  return (
    <nav className="h-28 max-tablet-xl:h-20 max-Wide-mobile-l:h-14 w-full flex flex-row-reverse items-center justify-between">
      <div className="flex flex-row-reverse items-center justify-between">
        <div
          onClick={jh}
          className="mr-2  max-Wide-mobile-l:mr-1 flex max-tablet-xl:mr-1 ml-11 text-4xl max-tablet-xl:text-xl pl-3 gap-1 max-tablet-xl:ml-3 max-Wide-mobile-l:text-xl cursor-pointer max-Wide-mobile-l:border-none  "
        >
          <GrBasket />
          <p className="text-xs rounded-full font-extrabold text-white size-5 flex justify-center items-center bg-neutral-400">
            {cartItems.length}
          </p>
        </div>
      ...
      <div
        className={`fixed flex flex-col justify-between top-0 ${close} transition-all w-96 max-mobile-xlk:w-64 h-[100vh] bg-slate-50 z-[100000000001]`}
      >
        <div className="">
          <div
            onClick={jh}
            className="w-full text-sky-600 text-2xl px-6 py-5 flex justify-between flex-row-reverse items-center font-bold font-dorna"
          >
            <span className="flex justify-center items-center max-mobile-xlk:text-lg">
              <GrClose /> <span className="w-1"></span>بستن
            </span>
            <span className="text-3xl text-black max-mobile-xlk:text-xl">
              سبد خرید
            </span>
          </div>
          ...
          {cartItems.length > 0 ? (
            cartItems.map((cartItem, index) => {
              ...
              return (
                <div
                  key={index}
                  className="border p-4 flex-col flex items-center  w-full justify-around border-gray-300"
                >
                  ...
                  <div className="flex-wrap flex w-full mt-4  flex-row justify-around items-center">
                    ...
                    <div className="max-mobile-xlk:mt-3 justify-center items-center text-lg font-semibold  flex flex-row">
                      {Math.floor(
                        (((product.money / 100) * (100 - product.offer)) /
                          1000) *
                          1000 *
                          cartItem.quantity
                      ).toLocaleString()}{" "}
                      تومان
                      <div className="w-10 h-9 mr-5 rounded flex justify-center items-center border text-sm border-black">
                        {cartItem.color}
                      </div>
                    </div>
                  </div>
        ...
        <div className="flex justify-center flex-col border-t border-black items-center mb-4">
          {cartItems.map((data, index) => {
            ...
            c +=
              (((product.money / 100) * (100 - product.offer)) / 1000) *
              1000 *
              data.quantity;
          })}
          ...
          <Link
            href={"/buyBasket"}
            className="w-11/12 h-16 bg-sky-500 text-white rounded-full flex justify-center items-center text-2xl font-dorna font-bold"
          >
            ثبت نهایی
          </Link>
        </div>
